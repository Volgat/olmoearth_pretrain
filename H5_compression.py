...
try:
    ...
    print("Warning: hdf5plugin not found. Additional compression filters may not be available.")
    hdf5plugin = None
...
def create_h5_file_with_multiple_datasets(file_path, datasets_dict, strategy_chunks_config, strategy_compression, strategy_compression_opts):
    """
    ...
    with h5py.File(file_path, 'w') as hf:
        for ds_name, ds_data in datasets_dict.items():
            ...
            if strategy_chunks_config == 'dataset_shape': # Indicates single chunk per dataset
            ...
            if strategy_compression is not None and current_chunks_config is None and strategy_chunks_config != True:
...
def verify_data_dictionaries(original_dict, loaded_dict):
    ...
    for key in original_dict:
        if not np.array_equal(original_dict[key], loaded_dict[key]):
            ...
            print(f"    Original shape: {original_dict[key].shape}, Loaded shape: {loaded_dict[key].shape}")
            return False
    print("  Data verification successful (all dataset names, shapes, and content match).")
    return True

def main():
    print("HDF5 Compression Strategy I/O Performance Comparison (Multiple Source Files - Averaged Results)\n")
    ...
    if not input_h5_file_paths or not input_h5_file_paths[0]: # Basic check
        print("Error: No input H5 file paths provided. Please update 'input_h5_file_paths' in the script.")
        return
    ...
    compression_strategies = [
        ...
        ("Gzip L1, Single Chunk pDS", 'dataset_shape', {'compression': 'gzip', 'compression_opts': 1}),
        ("Gzip L2, Single Chunk pDS", 'dataset_shape', {'compression': 'gzip', 'compression_opts': 2}),
        ("Gzip L3, Single Chunk pDS", 'dataset_shape', {'compression': 'gzip', 'compression_opts': 3}),
        ("Gzip L4, Single Chunk pDS", 'dataset_shape', {'compression': 'gzip', 'compression_opts': 4}),
        ("Gzip L9, Single Chunk pDS", 'dataset_shape', {'compression': 'gzip', 'compression_opts': 9}),
        ("Gzip L3, Auto-Chunking", True, {'compression': 'gzip', 'compression_opts': 3}),
        ("Gzip L4, Auto-Chunking", True, {'compression': 'gzip', 'compression_opts': 4}),
    ...
    if hdf5plugin:
        plugin_strategies = [
            ("Zstd L1, Single Chunk pDS", 'dataset_shape', {'compression': hdf5plugin.Zstd(clevel=1)}),
            ("Zstd L1, Auto-Chunking", True, {'compression': hdf5plugin.Zstd(clevel=1)}),
            ("Zstd L2, Single Chunk pDS", 'dataset_shape', {'compression': hdf5plugin.Zstd(clevel=2)}),
            ("Zstd L2, Auto-Chunking", True, {'compression': hdf5plugin.Zstd(clevel=2)}),
            ("Zstd L3, Single Chunk pDS", 'dataset_shape', {'compression': hdf5plugin.Zstd(clevel=3)}),
            ("Zstd L9, Single Chunk pDS", 'dataset_shape', {'compression': hdf5plugin.Zstd(clevel=9)}),
            ("Zstd L3, Auto-Chunking", True, {'compression': hdf5plugin.Zstd(clevel=3)}),

            ("Blosc L5 Sh(B), Single Chunk pDS", 'dataset_shape', {'compression': hdf5plugin.Blosc(clevel=5, shuffle=hdf5plugin.Blosc.SHUFFLE)}),
            ("Blosc L5 Sh(B), Auto-Chunking", True, {'compression': hdf5plugin.Blosc(clevel=5, shuffle=hdf5plugin.Blosc.SHUFFLE)}),

            ("Blosc2 L5 Sh(B), Single Chunk pDS", 'dataset_shape', {'compression': hdf5plugin.Blosc2(clevel=5, filters=hdf5plugin.Blosc2.SHUFFLE)}),
            ("Blosc2 L5 Sh(B), Auto-Chunking", True, {'compression': hdf5plugin.Blosc2(clevel=5, filters=hdf5plugin.Blosc2.SHUFFLE)}),

            ("BZip2 L5, Single Chunk pDS", 'dataset_shape', {'compression': hdf5plugin.BZip2(blocksize=5)}),
            ("BZip2 L5, Auto-Chunking", True, {'compression': hdf5plugin.BZip2(blocksize=5)}),

            ("LZ4 L0, Single Chunk pDS", 'dataset_shape', {'compression': hdf5plugin.LZ4()}),
            ("LZ4 L0, Auto-Chunking", True, {'compression': hdf5plugin.LZ4()}),

            ("Bitshuffle(St), Single Chunk pDS", 'dataset_shape', {'compression': hdf5plugin.Bitshuffle()}),
            ("Bitshuffle(St), Auto-Chunking", True, {'compression': hdf5plugin.Bitshuffle()}),
            ("Bitshuffle+LZ4, Auto-Chunk", True, {'compression': hdf5plugin.Bitshuffle(lz4=True)}),
            ("Bitshuffle+Zstd, Auto-Chunk", True, {'compression': hdf5plugin.Bitshuffle(cname='zstd', clevel=3)}),
        ]
    ...
    for file_idx, input_h5_file_path in enumerate(input_h5_file_paths):
        ...
        try:
            ...
            if not source_datasets_dict:
                print(f"Warning: No data loaded from source H5 file {input_h5_file_path}. Skipping this file.")
                continue
        except Exception as e:
            print(f"Error loading source data from {input_h5_file_path}: {e}. Skipping this file.")
            continue

        total_source_data_size_mb = sum(arr.nbytes for arr in source_datasets_dict.values()) / (1024*1024)
        print(f"Using data from {len(source_datasets_dict)} dataset(s) in '{input_h5_file_path}'.")
        print(f"Total size of loaded source data for this file: {total_source_data_size_mb:.2f} MB\n")

        with tempfile.TemporaryDirectory() as tmpdir:
            print(f"Using temporary directory for test H5 files for {os.path.basename(input_h5_file_path)}: {tmpdir}\n")

            for description, chunks_guidance, compression_kwargs in compression_strategies:
                print(f"--- Testing Strategy: {description} (for {os.path.basename(input_h5_file_path)}) ---")
                safe_desc = description.replace(' ', '_').replace('(', '').replace(')', '').replace(',', '').replace('-', '').replace('.', '_')
                file_name = f"test_{os.path.basename(input_h5_file_path)}_{safe_desc}.h5"
                temp_h5_file_path = os.path.join(tmpdir, file_name)

                try:
                    create_h5_file_with_multiple_datasets(
                        temp_h5_file_path, source_datasets_dict,
                        strategy_chunks_config=chunks_guidance,
                        strategy_compression=compression_kwargs.get('compression'),
                        strategy_compression_opts=compression_kwargs.get('compression_opts')
                    )
                    file_size_mb = os.path.getsize(temp_h5_file_path) / (1024 * 1024)
                    print(f"  Created temp H5 file: {temp_h5_file_path}, Size: {file_size_mb:.2f} MB")
                except Exception as e:
                    print(f"  Error creating temp H5 file {temp_h5_file_path} with strategy '{description}': {e}")
                    print(f"    Filter: {compression_kwargs.get('compression')}, Opts: {compression_kwargs.get('compression_opts')}, Chunks: {chunks_guidance}")
                    continue # Skip to next strategy for this file

                try:
                    load_time, loaded_dict_from_temp = time_h5_load_all_datasets(temp_h5_file_path)
                    print(f"  Time to load all datasets from temp file: {load_time:.4f}s")

                    is_lossy = compression_kwargs.get('compression') in ["sperr", "sz", "sz3", "zfp"] # Assuming these are identifier for lossy
                    if hasattr(compression_kwargs.get('compression'), 'filter_name'): # For hdf5plugin objects
                        is_lossy = compression_kwargs.get('compression').filter_name in ["sperr", "sz", "sz3", "zfp"]


                    if is_lossy:
                        print("  Skipping exact content verification for known lossy filter.")
                        if source_datasets_dict.keys() != loaded_dict_from_temp.keys():
                             print("  Data verification failed: Dataset names mismatch for lossy.")
                        else:
                            match = True
                            for key in source_datasets_dict:
                                if source_datasets_dict[key].shape != loaded_dict_from_temp[key].shape:
                                    print(f"  Data verification failed: Shape mismatch for lossy dataset '{key}'.")
                                    match = False
                                    break
                            if match:
                                 print("  Data verification basic (names, shapes) successful for lossy filter.")
                    else:
                        verify_data_dictionaries(source_datasets_dict, loaded_dict_from_temp)
                    ...
                    if description not in aggregated_results_data:
                        aggregated_results_data[description] = {
                            ...
                            "comp_opts": str(compression_kwargs.get('compression_opts')),
                            "chunks_guidance": str(chunks_guidance)
                        }

                    aggregated_results_data[description]["load_times"].append(load_time)
                    aggregated_results_data[description]["file_sizes_mb"].append(file_size_mb)
                    aggregated_results_data[description]["successful_runs"] += 1

                except Exception as e:
                    print(f"  Error loading/verifying data from temp H5 file {temp_h5_file_path}: {e}")
                print("-" * 30)
    ...
    if final_summary_results:
        ...
        results_sorted_by_load_time = sorted(final_summary_results, key=lambda x: x['load_time'])
        ...
        for res in results_sorted_by_load_time:
            ...
            if hasattr(res.get('compression'), 'filter_name'): # Check for hdf5plugin objects
            ...
            if len(display_opts_str) > 23: display_opts_str = display_opts_str[:20] + "..."
            display_compression_str = compression_str
            if len(display_compression_str) > 28: display_compression_str = display_compression_str[:25] + "..."

            print(f"{res['description']:<50} | {res['load_time']:<15.4f} | {res['file_size_mb']:<15.2f} | {num_files_str:<10} | {display_compression_str:<30} | {display_opts_str:<25} | {chunks_guidance_str:<20}")

        # Sort by Average File Size
        results_sorted_by_file_size = sorted(final_summary_results, key=lambda x: x['file_size_mb'])
        ...
        for res in results_sorted_by_file_size:
            ...
            print(f"{res['description']:<50} | {res['file_size_mb']:<15.2f} | {res['load_time']:<15.4f} | {num_files_str:<10} | {display_compression_str:<30} | {display_opts_str:<25} | {chunks_guidance_str:<20}")
    else:
        print("No results to display. Check for errors during file processing or if no strategies had successful runs.")
